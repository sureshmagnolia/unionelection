...
// ------------------------------------------------
        ...
        loadBooths: () => {
            ...
            onSnapshot(collection(db, "booths"), (snapshot) => {
                ...
                booths.forEach(b => {
                    ...
                    tr.innerHTML = `
                        <td>
                            <span class="fw-bold">${b.name}</span><br>
                            <small class="text-muted">${b.location}</small>
                        </td>
                        <td>
                            <span class="badge bg-primary fs-6">${b.voterCount || 0}</span>
                            <div class="small text-muted mt-1" style="max-width:200px; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">
                                ${b.assignedDepts ? b.assignedDepts.join(', ') : '-'}
                            </div>
                        </td>
                        <td>
                            <small>Start: <strong>${b.serialStart || '-'}</strong></small><br>
                            <small>End: <strong>${b.serialEnd || '-'}</strong></small>
                        </td>
                        <td>
        ...
        // 3. AUTO ASSIGN LOGIC (The Brain)
        autoAssignBooths: async () => {
            if(!confirm("⚠️ This will reset current allocations and redistribute ALL students to booths based on Departments. Continue?")) return;

            // Step A: Get all Booths
            ...
            // Step D: Distribute Depts to Booths (Round Robin to balance load)
            // Note: This assumes booths have capacity. 
            // We fill Booth 1, then Booth 2, etc.
            
            let boothIndex = 0;
            const batch = writeBatch(db);
            const boothUpdates = {}; // Track updates for booth docs

            Object.keys(deptMap).forEach(deptName => {
                const deptStudents = deptMap[deptName];
                const targetBooth = booths[boothIndex];

                // Assign students in this dept to this booth
            ...
            // Prepare Booth Updates
            booths.forEach(b => {
                const bRef = doc(db, "booths", b.id);
                batch.update(bRef, {
                    voterCount: b.currentLoad,
                    assignedDepts: b.depts
                });
            });

            try {
                await batch.commit();
                alert(`Success! Distributed ${students.length} students across ${booths.length} booths.`);
            } catch(e) {
                console.error(e);
